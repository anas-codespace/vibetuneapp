...
export function BulkAddToPlaylistSheet({ open, onClose, tracks, onDone }: Props) {
  const listFn = useServerFn(getMyPlaylists);
  const addFn = useServerFn(bulkAddToPlaylist);
  ...
  const runAdd = async (playlistId: string, label: string) => {
    if (!tracks.length) return;
    setBusy(true);
    try {
      const res = await addFn({ data: { playlistId, tracks } });
      toast.success(`Added ${res.added} to "${label}"`, {
        description: res.skipped ? `${res.skipped} already in playlist` : undefined,
      });
          ...
          >
            <div className="flex items-center justify-between px-5 pt-4 pb-2">
              <div>
                ...
                <p className="text-xs text-white/50">{tracks.length} selected</p>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="max-h-[50vh] overflow-y-auto px-3 pb-6">
              ...
              {(playlists ?? []).map((p) => (
                <button
                  key={p.id}
                  disabled={busy}
                  onClick={() => runAdd(p.id, p.name)}
                  className="flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-left transition hover:bg-white/5 disabled:opacity-50"
                >
